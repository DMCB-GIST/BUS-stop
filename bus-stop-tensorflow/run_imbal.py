# -*- coding: utf-8 -*-
"""
Created on Fri Sep 17 05:27:58 2021
...
arg_parser.add_argument('--K',default=100,required=False)
arg_parser.add_argument('--test_K',default=1000,required=False)
arg_parser.add_argument('--trn_neg',default=0.5,required=False)
arg_parser.add_argument('--tst_neg',default=0.5,required=False)
arg_parser.add_argument('--epochs',default=30,required=False)
...
K = int(arg_saved.K)
test_K = int(arg_saved.test_K)
trn_neg = float(arg_saved.trn_neg)
tst_neg = float(arg_saved.tst_neg)
if data=='DBPedia':
    max_seq_length = 128
elif data=='Elec':
    max_seq_length = 256
...
trn_pos_k = K - int(K*trn_neg)
trn_neg_k = int(K*trn_neg)
tst_pos_k = tst_len - int(tst_len*tst_neg)
tst_neg_k = int(tst_len*tst_neg)
neg_label = label_list[0]
pos_label = label_list[1]

saver = tf.train.Saver()

dump_path = data_path+'/logs/K'+str(K)+'-TR'+str(trn_neg)+'-TS'+str(tst_neg)
if not os.path.isdir(dump_path):
    os.makedirs(dump_path)

si_str = '_si'+str(start)+'to'+str(end)
mylog = fs.my_logging(dump_path+'/skewed_log_at_seed'+si_str+'.log')
...
for si in range(start,end):
    ...
    full_tst_examples = fs.tsv2examples(os.path.join(data_path,'test.tsv'))
    
    rand_indx = [i for i in range(len(full_trn_examples))]
    random.Random(si).shuffle(rand_indx)
    tst_rand_indx = [i for i in range(len(full_tst_examples))]
    random.Random(si).shuffle(tst_rand_indx)
    
    mylog.printNlogging(' ')
    mylog.printNlogging(data)
    mylog.printNlogging('si:',si)
    mylog.printNlogging('Random_trn_indx:',np.array(rand_indx))
    mylog.printNlogging('Random_tst_indx:',np.array(tst_rand_indx))
    mylog.printNlogging(' ')
    ...
    for ri in tst_rand_indx:
        example = full_tst_examples[ri]
        tst_examples_per_class[example.label].append(example)
        
    trn_examples = examples_per_class[pos_label][:trn_pos_k]
    trn_examples+= examples_per_class[neg_label][:trn_neg_k]
    
    vlp_examples = examples_per_class[pos_label][trn_pos_k:trn_pos_k+25]
    vlp_examples+= examples_per_class[neg_label][trn_neg_k:trn_neg_k+25]
    
    tst_examples = tst_examples_per_class[pos_label][:tst_pos_k]
    tst_examples+= tst_examples_per_class[neg_label][:tst_neg_k]
    ...
    mylog.printNlogging('train:',fs.label_counter(trn_examples,label_list))
    mylog.printNlogging('val_p:',fs.label_counter(vlp_examples,label_list))
    mylog.printNlogging('test:',fs.label_counter(tst_examples,label_list))
    mylog.printNlogging(' ')
    ...
    for pi in range(pe_split):
        ...
        for base_i in range(N_base):
            ...
            if pi==0 and base_i==0:
                ...
                l_tst_f1 = fs.f1_score(tst.label_ids,tst_probs.argmax(1),average='macro',zero_division=1)
                print(' ')
                print('val-stop local tst acc/f1/loss/ece/oe =',
                      fs.rounds([l_tst_acc,l_tst_f1,l_tst_loss,l_tst_ece,l_tst_oe]))
                print(' ')
                sess.close()
        ...
        pre_dev_f1 = fs.f1_score(pe_dev.label_ids,pre_dev_probs[-1].argmax(1),average='macro',zero_division=1)
        pre_dev_acc, pre_dev_loss = list(eval_acc_loss('pe_dev'))
        pre_dev_perfs.append([pre_dev_acc, pre_dev_f1, pre_dev_loss])
        if pi==0:
            tst_probs = ulb_probs
            g_tst_acc,g_tst_loss,g_tst_ece,g_tst_oe = eval_all_metrics(tst_probs,'tst',bins=15)
            g_tst_f1 = fs.f1_score(tst.label_ids,tst_probs.argmax(1),average='macro',zero_division=1)
            #g_tst_acc,g_tst_loss = eval_acc_loss('tst')
            print(' ')
            print('val-stop global tst acc/f1/loss/ece/oe =',
                  fs.rounds([g_tst_acc,g_tst_f1,g_tst_loss,g_tst_ece,g_tst_oe]))
            print(' ')
        sess.close()
    ...
    fs.dump_all(np.array(pre_dev_confs_over_seeds),dump_path+'/skewed_pre_dev_confs'+si_str+'.bin')
    fs.dump_all(np.array(pre_ulb_probs_over_seeds),dump_path+'/skewed_pre_ulb_probs'+si_str+'.bin')
    fs.dump_all(np.array(pre_dev_probs_over_seeds),dump_path+'/skewed_pre_dev_probs'+si_str+'.bin')
    fs.dump_all(np.array(pre_dev_perfs_over_seeds),dump_path+'/skewed_pre_dev_perfs'+si_str+'.bin')
    fs.dump_all(np.array(pre_pe_epochs_over_seeds),dump_path+'/skewed_pre_pe_epochs'+si_str+'.bin')
    ...
    print('val-'+val_per_class+'-stop local tst acc/f1/loss/ece/oe =',fs.rounds([l_tst_acc,l_tst_f1,l_tst_loss,l_tst_ece,l_tst_oe]))
    print('val-'+val_per_class+'-stop global tst acc/f1/loss/ece/oe =',fs.rounds([g_tst_acc,g_tst_f1,g_tst_loss,g_tst_ece,g_tst_oe]))
    print(50*'#')
    print(' ')
    ...
    for base_i in range(N_base):
        ...
        print_columns = ['epoch','time(m)','trn_a','trn_l',
                         'tst_a','tst_f1','tst_l','tst_ece','tst_oe',
        ...
        for bi in range(trn_bnum):
            if (bi*btchs)//trn_len==epoch:
                ...
                tst_f1 = fs.f1_score(tst.label_ids,tst_probs.argmax(1),average='macro',zero_division=1)
                ...
                record = [epoch,round((time.time() - start_t)/60.0,2),
                          trn_acc,trn_loss,tst_acc,tst_f1,tst_loss,tst_ece,tst_oe,
    ...
    fs.dump_all(np.array(all_records_over_seeds),dump_path+'/skewed_all_records'+si_str+'.bin')
    fs.dump_all(np.array(all_u_probs_over_seeds),dump_path+'/skewed_ulb_records'+si_str+'.bin')
    ...
    mylog.printNlogging('pe_stop_epoch =',int(pe_stop_epoch+0.5))
    val_per_class = str(int(len(trn_examples)*pe_dev_ratio/num_labels))
    mylog.printNlogging('val-'+val_per_class+'-stop local tst acc/f1/loss/ece/oe =',
                        fs.rounds([l_tst_acc,l_tst_f1,l_tst_loss,l_tst_ece,l_tst_oe]))
    mylog.printNlogging('val-'+val_per_class+'-stop global tst acc/f1/loss/ece/oe =',
                        fs.rounds([g_tst_acc,g_tst_f1,g_tst_loss,g_tst_ece,g_tst_oe]))
    
    val_per_class = str(int(vlp_len/num_labels))
    mylog.printNlogging('val+'+val_per_class+'-stop local tst acc/f1/loss/ece/oe =',
                        fs.rounds([vl_tst_acc,vl_tst_f1,vl_tst_loss,vl_tst_ece,vl_tst_oe]))
    mylog.printNlogging('val+'+val_per_class+'-stop global tst acc/f1/loss/ece/oe =',
